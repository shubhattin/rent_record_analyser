...
export const get_year_list = (rent_data: rent_data_type) => {
  ...
  for (let dt of rent_data) {
    const yr = parseInt(dt.month.split('-')[0]);
...
export const get_month_list = (rent_data: rent_data_type, year: number) => {
  ...
  const amounts: number[] = [];
  for (let dt of rent_data) {
    if (year !== parseInt(dt.month.split('-')[0])) continue;
    const month = parseInt(dt.month.split('-')[1]);
...
export const get_date_list = (rent_data: rent_data_type, year: number, month: number) => {
  ...
  for (let dt of rent_data) {
    if (year !== parseInt(dt.month.split('-')[0])) continue;
    if (month !== parseInt(dt.month.split('-')[1])) continue;

    const index = (() => {
      const date = dt.date;
      for (let i = 0; i < date_records.length; i++) if (date_records[i][0].date === date) return i;
      return -1;
    })();
    if (index === -1) {
